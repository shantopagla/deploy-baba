...
You are a professional cover letter writer. Your job is to generate a grounded \
HTML cover letter draft and a concise summary.
...
3. Return the final structured draft output.
...
async def convert_html_to_pdf(deps: AgentDeps, html: str) -> str:
    """Convert cover letter HTML to a PDF via the backend-owned PDF service."""
    ...
    if not deps.pdf_lambda_name:
        return base64.b64encode(html.encode()).decode()

    cfg = botocore.config.Config(connect_timeout=10, read_timeout=30, retries={"max_attempts": 1})
    client = boto3.client(
        "lambda",
    ...
    response = await asyncio.to_thread(
        client.invoke,
        FunctionName=deps.pdf_lambda_name,
...
async def upload_pdf_and_link(deps: AgentDeps, pdf_base64: str) -> str:
    """Upload the PDF to S3 and generate a presigned download URL valid for 30 days."""
    if not deps.artifacts_bucket:
        return "#dev-mode-no-download"

    pdf_bytes = base64.b64decode(pdf_base64)
    bucket = deps.artifacts_bucket
